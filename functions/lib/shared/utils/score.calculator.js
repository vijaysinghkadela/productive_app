...
function calculateProductivityScore(input) {
    ...
    const totalDeductions = socialMediaDeduction +
        screenTimeDeduction +
        overrideDeduction +
    ...
    if (allGoalsMet && activeGoals.length > 0)
        goalBonus += 10;
    ...
    const morningRoutineBonus = dailyStats.firstPhoneUse && dailyStats.firstPhoneUse >= '08:00' ? 5 : 0;
    const totalAdditions = focusBonus +
        goalBonus +
        habitBonus +
...
function checkLevelUp(previousXp, newXp) {
...
function calculateSessionXp(durationMinutes, completionRate, distractionCount) {
    const base = 50;
