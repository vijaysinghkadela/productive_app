...
export const createHabit = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  const habitRef = db.collection(Collections.USERS).doc(uid).collection(Collections.HABITS).doc();
...
export const trackHabit = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
