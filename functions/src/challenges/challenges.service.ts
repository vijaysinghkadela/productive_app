...
export const joinChallenge = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  if (!['pro', 'elite', 'lifetime'].includes(tier)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Challenges require Pro or Elite subscription',
    );
  }
  ...
  if (challenge.status !== 'active' && challenge.status !== 'upcoming') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Challenge is not accepting participants',
    );
  }
...
export const withdrawChallenge = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
  ...
  if (snap.data()?.status !== 'active') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Cannot withdraw from completed/failed challenge',
    );
  }
...
export const getChallengeDetails = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
        ...
        for (const pDoc of participantsSnap.docs) {
          ...
          if (completed) {
            await checkAndUnlockAchievements(p.userId, 'challenge_completed', {
              challengesCompleted: (
                await db.collection(Collections.CHALLENGES).where('status', '==', 'completed').get()
              ).size, // approximate
            });
          }
        }
