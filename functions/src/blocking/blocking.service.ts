  ...
  .https.onCall(async (data, context) => {
    ...
    if (!name || !Array.isArray(appIds) || !Array.isArray(schedule)) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'name, appIds, and schedule are required',
      );
    }
    ...
    const existingSchedules = await db
      ...
      .doc(uid)
      .collection(Collections.BLOCKING_SCHEDULE)
      .where('status', '==', 'active')
      .get();
    ...
    if (strictMode && !['pro', 'elite', 'lifetime'].includes(tier)) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Strict mode requires Pro or Elite subscription',
      );
    }
...
export const createFocusMode = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
  const now = Timestamp.now();
  ...
  if (!['pro', 'elite', 'lifetime'].includes(tier)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Focus modes require Pro or Elite subscription',
    );
  }
  ...
  const modeRef = db
    .collection(Collections.USERS)
    .doc(uid)
    .collection(Collections.FOCUS_MODES)
    .doc();
...
export const getActiveBlocks = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
...
export const logOverrideAttempt = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
