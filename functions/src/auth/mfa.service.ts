...
export const generateTOTPSecret = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    ...
    const mfaDoc = await db.collection(Collections.USERS).doc(uid)
      .collection('mfa').doc('totp').get();
...
export const verifyTOTPToken = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    if (!isValid) {
      ...
      await db.collection(Collections.USERS).doc(uid)
        .collection('mfa').doc('totp').update({
    ...
    if (isSetup) {
      ...
      await db.collection(Collections.USERS).doc(uid)
        .collection('mfa').doc('backup_codes').set({
          codes: hashedCodes,
          usedCount: 0,
...
export const sendSMSCode = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    const smsDoc = await db.collection(Collections.USERS).doc(uid)
      .collection('mfa').doc('sms').get();
    ...
    await db.collection(Collections.USERS).doc(uid)
      .collection('mfa').doc('sms').set({
        ...
        hourCountStartedAt: smsDoc.exists && smsDoc.data()!.hourCountStartedAt
          ? smsDoc.data()!.hourCountStartedAt : Timestamp.now(),
      }, { merge: true });
...
export const verifyBackupCode = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    const backupDoc = await db.collection(Collections.USERS).doc(uid)
      .collection('mfa').doc('backup_codes').get();
...
export const disableMFA = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    const { token } = data;
    if (!token) throw new functions.https.HttpsError('invalid-argument', 'Current TOTP token required to disable MFA');
