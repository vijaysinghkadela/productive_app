...
export const syncDailyUsage = functions
  .region(REGION)
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (!existing.exists) {
        ...
        (updateData as DailyStatsDocument).focusSessions = {
          completed: 0, abandoned: 0, totalMinutes: 0, averageLength: 0, longestSession: 0,
        };
        ...
        (updateData as DailyStatsDocument).sleepData = {
          bedtime: null, wakeTime: null, quality: null, lateNightUsageMinutes: 0,
        };
        (updateData as DailyStatsDocument).productivityScore = {
          final: 0,
          components: {
            baseScore: 100, socialMediaDeduction: 0, screenTimeDeduction: 0,
            overrideDeduction: 0, abandonedSessionDeduction: 0, habitDeduction: 0,
            focusBonus: 0, goalBonus: 0, habitBonus: 0, streakBonus: 0,
            journalBonus: 0, morningRoutineBonus: 0, socialMediaFreeBonus: 0,
          },
          hourlySnapshots: [],
          calculatedAt: now,
        };
...
export const getUsageAnalytics = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    if (compareWithPrevious) {
      ...
      const prevSnap = await db.collection(Collections.USERS).doc(uid)
        .collection(Collections.DAILY_STATS)
      ...
      comparison = {
        screenTimeChange: prevScreenTime > 0
          ? Math.round(((totalScreenTime - prevScreenTime) / prevScreenTime) * 100) : 0,
        socialMediaChange: prevSocial > 0
          ? Math.round(((totalSocialMedia - prevSocial) / prevSocial) * 100) : 0,
    ...
    return {
      ...
      dailyAvgScreenTime: stats.length > 0
        ? Math.round(totalScreenTime / stats.length) : 0,
      topApps,
      hourlyHeatmap,
      scores: stats.map((s) => ({
        date: s.date,
