...
import { getFirestore, getMessaging, REGION } from '../shared/config/firebase.config';
import { Collections } from '../shared/constants/collections.constants';
import { UserDocument, NotificationType } from '../shared/types/firestore.types';
...
export async function sendNotification(input: SendNotificationInput): Promise<void> {
  ...
  const typeToSetting: Record<string, boolean> = {
  ...
  if (typeToSetting[type] === false) return;
  ...
  if (prefs.quietHoursStart && prefs.quietHoursEnd) {
    ...
    if (qStart > qEnd) {
      ...
      if (nowTime >= qStart || nowTime < qEnd) {
        if (type !== 'system') return; // Don't send during quiet hours (except system)
      }
    } else {
      if (nowTime >= qStart && nowTime < qEnd) {
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      if (user.stats.lastActiveDate === today) {
        ...
        if (dailySnap.exists) {
          ...
          if (stats.focusSessions?.completed > 0) continue; // Already focused
        }
      }
...
export const NotificationTemplates = {
  ...
  sessionComplete: (minutes: number, xp: number) => ({
  ...
  partnerActivity: (partnerName: string, action: string) => ({
