...
import {
  AccountabilityPairDocument,
  AccountabilityGroupDocument,
} from '../shared/types/firestore.types';
import { sendNotification } from '../notifications/notifications.service';

// ─── Invite Accountability Partner (Callable) ───
export const invitePartner = functions.region(REGION).https.onCall(async (data, context) => {
  ...
  if (!partnerUsername && !inviteCode) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'partnerUsername or inviteCode required',
    );
  }
  ...
  if (!['pro', 'elite', 'lifetime'].includes(tier)) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Accountability requires Pro or Elite subscription',
    );
  }
...
export const acceptPartner = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
  ...
  // Update pair
  const updatedUserIds: [string, string] =
    pair.userIds[1] === '' ? [pair.userIds[0], uid] : pair.userIds;
...
// ─── Send Nudge (Callable) ───
export const sendNudge = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
...
export const createAccountabilityGroup = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!['elite', 'lifetime'].includes(tier)) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Groups require Elite subscription',
      );
    }
