...
export function calculateStreak(
  history: { date: string; completed: boolean; skipped: boolean }[],
  today: string,
): StreakResult {
  if (history.length === 0) {
    ...
    if (entry.completed || entry.skipped) {
      ...
      if (lastDate) {
        const dayDiff = Math.floor(
          (entryDate.getTime() - lastDate.getTime()) / (24 * 60 * 60 * 1000),
        );
        if (dayDiff === 1) {
          tempStreak++;
        } else if (dayDiff > 1) {
  ...
  for (const entry of sorted) {
    ...
    const dayDiff = Math.floor(
      (expectedDate.getTime() - entryDate.getTime()) / (24 * 60 * 60 * 1000),
    );
