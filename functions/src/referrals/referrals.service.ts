...
import { ReferralDocument } from '../shared/types/firestore.types';
import { sendNotification } from '../notifications/notifications.service';
import { checkAndUnlockAchievements } from '../achievements/achievements.engine';

// ─── Create Referral Link (Callable) ───
export const createReferralLink = functions.region(REGION).https.onCall(async (data, context) => {
...
export const processReferral = functions.region(REGION).https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
  const uid = context.auth.uid;
  const db = getFirestore();
