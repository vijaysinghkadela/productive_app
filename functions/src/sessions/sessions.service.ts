...
export const createSession = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    await db.collection(Collections.USERS).doc(uid)
      .collection(Collections.SESSIONS).doc(sessionId).set(session);
...
export const completeSession = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // Fetch session
    const sessionRef = db.collection(Collections.USERS).doc(uid)
      .collection(Collections.SESSIONS).doc(input.sessionId);
    ...
    await sessionRef.update({
      ...
      endedAt: now,
      updatedAt: now,
      ...(input.phases && { phases: input.phases.map((p) => ({
        ...p,
...
export const getSessionAnalytics = functions
  .region(REGION)
  .https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Not authenticated');
    const uid = context.auth.uid;
    ...
    const sessionsSnap = await db.collection(Collections.USERS).doc(uid)
    ...
    const avgLength = completedSessions.length > 0
      ? totalMinutes / completedSessions.length : 0;
